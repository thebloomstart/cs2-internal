...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col, const ImVec4& rarity_col, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
...
bool ImGui::ImageButton(const char* str_id, ImTextureID user_texture_id, const ImVec2& size, const ImVec4& rarity_col, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    return ImageButtonEx(window->GetID(str_id), user_texture_id, size, uv0, uv1, bg_col, tint_col, rarity_col);
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth();
    const float h = GetFrameHeight() * 0.8f;
    ...
    // Add glow as an outline (shadow)
    window->DrawList->AddShadowRect(frame_bb.Min, frame_bb.Max, IM_COL32(0, 0, 0, 200) , 32.f, ImVec2(0, 0), 11, ImDrawFlags_RoundCornersAll);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, IM_COL32(15, 15, 15, 255), 5.f);
  ...
  //  RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, g.Style.FrameRounding);
    ...
    GetWindowDrawList()->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(c::elements::background), c::elements::rounding);

    GetWindowDrawList()->AddRectFilled(total_bb.Min + ImVec2(w - 140, 0), total_bb.Max, GetColorU32(c::elements::background), c::elements::rounding);
    GetWindowDrawList()->AddRectFilledMultiColor(total_bb.Min, total_bb.Max - ImVec2(120, 0), VGetColorU32(c::elements::background, 0.f), VGetColorU32(c::elements::background, 1.f), VGetColorU32(c::elements::background, 1.f), VGetColorU32(c::elements::background, 0.f));
  ...
  //  GetWindowDrawList()->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + 9, grab_bb.Max.y), VGetColorU32(c::accent, 1.f), 2.f);
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    // Push a specific font for rendering the text
	PushFont(NULL);
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.7f));
    PopFont();
    if (hovered)
        window->DrawList->AddShadowRect(frame_bb.Min, frame_bb.Max, IM_COL32(0, 0, 0, 200), 32.f, ImVec2(0, 0), 11, ImDrawFlags_RoundCornersAll);
    ...
    if (label_size.x > 0.0f) {
        PushStyleColor(ImGuiCol_Text, ImVec4( 73.f / 255.f  , 73.f / 255.f, 73.f / 255.f, 0.90f));  // Replace with your desired color

        RenderText(ImVec2(frame_bb.Min.x, total_bb.Min.y + style.FramePadding.y - 4), label);
        PopStyleColor();
    }
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags | (temp_input_allowed ? ImGuiItemStatusFlags_Inputable : 0));
    return value_changed;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    PushFont(font::lexend_bold);
    {
        ...
        if (render_cursor || render_selection)
        {
            ...
            if (render_selection)
            {
                ...
                ImU32 bg_color = VGetColorU32(c::accent, 0.1f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
    ...
    GetWindowDrawList()->AddRectFilledMultiColor(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(size_arg.y - CalcTextSize(hint).y - 1, 1), VGetColorU32(c::elements::background, 0.f), VGetColorU32(c::elements::background, 1.f), VGetColorU32(c::elements::background, 1.f), VGetColorU32(c::elements::background, 0.f));
...
// - Selectable()
//-------------------------------------------------------------------------
bool ImGui::ImageSelectable(const char* label, ImTextureID texture_id , bool selected, ImGuiSelectableFlags flags, const ImVec2& size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Handle mouse input using ImageButton
    if (!disabled_item && (flags & ImGuiSelectableFlags_NoHoldingActiveID) == 0) {
        const ImVec2 uv0 = ImVec2(0.0f, 0.0f);
        const ImVec2 uv1 = ImVec2(1.0f, 1.0f);
        pressed = ImageButton(label, texture_id, size);
    }

    // Render
    const ImU32 tint_col = GetColorU32(ImGuiCol_Button);
...
bool ImGui::ImageSelectable(const char* label, ImTextureID textureID, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    if (ImageSelectable(label, textureID, *p_selected, flags, size_arg))
    {
