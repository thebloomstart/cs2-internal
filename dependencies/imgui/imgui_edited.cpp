...
#include <string>
    ...
    bool Keybind(const char* label, const char* description, int* key, int* mode)
    {
        ...
        GetWindowDrawList()->AddShadowRect(rect.Min, rect.Max, VGetColorU32(ImColor(0, 0, 0, 155)), 24.f * dpi, ImVec2(0, 0), ImDrawFlags_RoundCornersAll, c::elements::rounding);

        RenderTextColor(font::lexend_regular, rect.Min + ImVec2(10, 0), rect.Max, GetColorU32(c::elements::text), description, ImVec2(0.0, 0.8));

        GetWindowDrawList()->AddRectFilled(rect.Min + ImVec2(width - 40, 0), rect.Max, GetColorU32(c::elements::background), c::elements::rounding);
        ...
        GetWindowDrawList()->AddShadowRect(clickable.Min, clickable.Max, VGetColorU32(ImColor(0, 0, 0, 155)), 8.f * dpi, ImVec2(0, 0), ImDrawFlags_RoundCornersAll, c::elements::rounding);
    ...
    bool edited::Tab(bool selected, const char* icon, const char* label, const char* description, const ImVec2& size_arg)
    {
        ...
        ImVec2 menuPos = ImVec2(pos.x, pos.y);
        ImVec2 menuSize = ImVec2(190, 400);

        // Draw the menu background
        GetWindowDrawList()->AddRectFilled(menuPos, menuPos + menuSize, VGetColorU32(c::tab::bg, 1.f), c::elements::rounding, ImDrawFlags_RoundCornersAll);
        GetWindowDrawList()->AddShadowRect(menuPos, menuPos + menuSize, VGetColorU32(ImVec4(0.f, 0.f, 0.f, 1.f), 1.f), 12.f, ImVec2(0, 0), c::elements::rounding, ImDrawFlags_RoundCornersAll);

        //  GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(5, 0), bb.Max - ImVec2(2, 2), VGetColorU32(c::tab::tab_active, state.bg_alpha), c::elements::rounding);
        GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(size.x / 2, 2), bb.Max - ImVec2(0, 4.f), VGetColorU32(state.shadow, 0.f), VGetColorU32(state.shadow, 1.f), VGetColorU32(state.shadow, 1.f), VGetColorU32(state.shadow, 0.f));
...
//        GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(size.x / 2, 0), bb.Max, VGetColorU32(state.shadow, 0.f), VGetColorU32(state.shadow, 1.f), VGetColorU32(state.shadow, 1.f), VGetColorU32(state.shadow, 0.f));
    ...
    bool edited::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
    {
       ...
       GetWindowDrawList()->AddRectFilled(parent_window->DC.CursorPos, parent_window->DC.CursorPos + size_arg, GetColorU32(c::background::filling), c::background::rounding, ImDrawFlags_RoundCornersAll);
       GetWindowDrawList()->AddRect(parent_window->DC.CursorPos, parent_window->DC.CursorPos + size_arg, VGetColorU32(ImColor(0,0,0, 55), 1.f), c::background::rounding, ImDrawFlags_RoundCornersAll);

       GetWindowDrawList()->AddShadowRect(parent_window->DC.CursorPos, parent_window->DC.CursorPos +
           size_arg, VGetColorU32(ImColor(0, 0, 0, 255), 1.f), 8.f * dpi, ImVec2(0, 0), 0, c::elements::rounding);
    ...
    bool edited::Checkbox(const char* label, const char* description, bool* v)
    {
        ...
        auto walpha = GetColorU32(ImVec4(c::elements::background.x, c::elements::background.y, c::elements::background.z, 1.f));
        auto nalpha = GetColorU32(ImVec4(c::elements::background.x, c::elements::background.y, c::elements::background.z, 0.1f));
        GetWindowDrawList()->AddShadowRect(rect.Min, rect.Max, GetColorU32(state.background), 8.f * dpi, ImVec2(0, 0), ImDrawFlags_RoundCornersAll, 2.f);
       ...
       */
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, nalpha, 3.f);

        
        PushClipRect(rect.Max - ImVec2(37, 37), rect.Max - ImVec2(13, 13), true);
        RenderCheckMark(GetWindowDrawList(), rect.Max - ImVec2(40 - (square_sz / 2) / 2, 40 - (square_sz / 2 + state.mark_pos) / 2), VGetColorU32(state.background, state.alpha), square_sz / 2);
        PopClipRect();
    ...
    bool SliderScalar(const char* label, const char* description, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
    {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::elements::background), 2.f);
        GetWindowDrawList()->AddShadowRect(frame_bb.Min, frame_bb.Max, VGetColorU32(ImColor(0, 0, 0, 155)), 8.f * dpi, ImVec2(0, 0), ImDrawFlags_RoundCornersAll, c::elements::rounding);
    ...
    bool BeginCombo(const char* label, const char* description, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
    {
        ...
        const ImRect rect(window->DC.CursorPos + ImVec2(140, (50 - 30) / 2), window->DC.CursorPos + ImVec2( 140, (50 + 30) / 2));
       ...
       // state.combo_size = ImLerp(state.combo_size, state.opened_combo ? (val * 33) + 17 : 0.f, g.IO.DeltaTime * 12.f);
        ...
        // cute glow outline
        GetWindowDrawList()->AddShadowRect(bb.Min, bb.Max, VGetColorU32(ImColor(0, 0, 0, 155)), 24.f * dpi, ImVec2(0, 0), ImDrawFlags_RoundCornersAll, c::elements::rounding);
        ...
        float offset = 125.0f; // «начение, на которое нужно сдвинуть квадрат вправо

        // ѕеремещаем только маленький квадрат (30x30 пикселей)
        GetWindowDrawList()->AddRectFilled(rect.Max - ImVec2(30 - offset, 30), rect.Max + ImVec2(offset, 0), GetColorU32(c::elements::background_widget), 2.f);
        ...
        // line of separation
		GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(480, 0), bb.Min + ImVec2(483, 50), VGetColorU32(ImColor(9, 9, 9)), c::elements::rounding);
		...
		ImGuiWindowFlags window_flags = /*ImGuiWindowFlags_NoTitleBar*/ 0;

		ImGui::PushStyleColor(ImGuiCol_TitleBg, c::accent);
		ImGui::PushStyleColor(ImGuiCol_TitleBgActive, c::accent);//idk wnich state of window so apply everything
		ImGui::PushStyleColor(ImGuiCol_TitleBgCollapsed, c::accent);
