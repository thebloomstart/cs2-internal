...
void F::LEGITBOT::AIM::AimAssist(CBaseUserCmdPB* pUserCmd, C_CSPlayerPawn* pLocalPawn, CCSPlayerController* pLocalController)
{
	...
	if (!IPT::IsKeyDown(C_GET(int, Vars.nLegitbotActivationKey)) && !C_GET(bool, Vars.bLegitbotAlwaysOn))
		return;
	...
	for (int nIndex = 1; nIndex <= iHighestIndex; nIndex++)
	{
		...
		I::GameTraceManager->TraceShape2(&ray, pLocalPawn->GetEyePosition(), pPawn->GetGameSceneNode()->GetSkeletonInstance()->pBoneCache->GetOrigin(6), &filter, &trace);
...
//void F::LEGITBOT::AIM::FovVisualize(CBaseUserCmdPB* pUserCmd, C_CSPlayerPawn* pLocalPawn, CCSPlayerController* pLocalController)
//{
//	auto fov = std::hypotf(viewangles.x - vec.x, viewangles.y - vec.y);
//
//	if (pLocalController->IsPawnAlive())
//		return;
//
...
void F::LEGITBOT::AIM::draw()
{
	if (!C_GET(bool, Vars.AimVof))
		return;
	...
	if (I::Engine->IsConnected())
	{
		...
		float maxFov = 15.0f; // Максимальный FOV
		float screenHeight = ImGui::GetIO().DisplaySize.y; // Высота экрана
		...
		// Уменьшаем радиус для того, чтобы он не был слишком большим
		float scaleAdjustment = 0.35f; // Множитель для уменьшения радиуса
		radius *= scaleAdjustment;

		Color_t color = C_GET(Color_t, Vars.colVof);

		// Преобразуем Color_t в IM_COL32 (порядок ABGR)
		...
		drawList->AddCircle(screenCenter, radius, imColor, 100); // Используем преобразованный цвет
	}
	else
	{
		return;
