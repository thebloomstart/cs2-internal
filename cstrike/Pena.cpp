...
namespace F::AUTOWALL
{
	...
	bool F::AUTOWALL::c_auto_wall::FireBullet(data_t& data, float& dmg, bool& valid)
	{
		...
		I::GameTraceManager->Init(filter, data.m_local_pawn, PENMASK, 3, 7);
        ...
        */
		...
		if (trace_data.m_num_update > 0)
		{
			for (int i{}; i < trace_data.m_num_update; i++)
			{
				...
				reinterpret_cast<std::uintptr_t>(trace_data.m_pointer_update_value) + i * sizeof(UpdateValueT));
				...
				reinterpret_cast<std::uintptr_t>(trace_data.m_arr.data()) + sizeof(trace_arr_element_t) * (value->handleIdx & 0x7fffu)));
				/*
                ...
                */
				...
				{
					ScaleDamage2(game_trace.HitboxData->m_nHitGroup, data.m_target_pawn, data.m_wpn_data->m_flArmorRatio(), data.m_wpn_data->m_flHeadshotMultiplier(), &corrected_dmg);
