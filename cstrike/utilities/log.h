...
public:
	...
	class scope_timer {
	public:
		inline scope_timer(timer& t, const std::string& name)
			: timer_(t), name_(name) {
			timer_.start(name_);
		}

		inline ~scope_timer() {
			double duration = timer_.end(name_);
			//std::stringstream ss;
			//ss << "scope '" << name_ << "' took " << std::fixed
			//	<< std::setprecision(3) << duration << "ms";
			//log_message(ss.str(), log_level::debug);
		}
	...
	inline void start(const std::string& marker) {
		std::lock_guard<std::mutex> lock(mutex_);
		markers_[marker] = std::chrono::high_resolution_clock::now();
	}

	inline double end(const std::string& marker) {
		std::lock_guard<std::mutex> lock(mutex_);
		auto it = markers_.find(marker);
		if (it == markers_.end()) {
			return 0.0;
		}
	...
	inline scope_timer create_scope_timer(const std::string& name) {
		return scope_timer(*this, name);
	}

private:
	std::unordered_map<std::string, time_point> markers_;
	std::mutex mutex_;
};
