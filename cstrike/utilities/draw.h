...
namespace ImGui
{
	...
	bool Keybind(const char* label, int* key, int* mode, ImGuiComboFlags flags);
}
...
namespace D
{
	...
	std::string LowerText(std::string data);

	std::string UpperText(std::string data);

	void RenderText(const Vector_t& txt_pos, Color_t color, const int flags, const ImFont* font, std::string message, const int font_size);

	void Rect(Vector_t start_pos, Vector_t end_pos, Color_t color, float rounding);
	...
	void RadialGradient3D(Vector_t pos, float radius, Color_t in, Color_t out, bool one);
	void RectOutline(Vector_t start_pos, Vector_t end_pos, Color_t color, float thickness, float rounding);
