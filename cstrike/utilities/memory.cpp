...
bool MEM::Setup()
{
	...
	fnGetInventoryManager = reinterpret_cast<decltype(fnGetInventoryManager)>(FindPattern(CLIENT_DLL, CS_XOR("48 8d 05 ?? ?? ?? ?? c3 cc cc cc cc cc cc cc cc 8b 91 ?? ?? ?? ?? b8")));
	bSuccess &= (fnGetInventoryManager != nullptr);

	fnGetClientSystem = reinterpret_cast<decltype(fnGetClientSystem)>(FindPattern(CLIENT_DLL, CS_XOR("48 8d 05 ?? ?? ?? ?? c3 cc cc cc cc cc cc cc cc 48 89 5c 24 ?? 48 89 74 24 ?? 57 48 83 ec ?? 48 8b fa")));
	bSuccess &= (fnGetClientSystem != nullptr);

	SetMeshGroupMask = reinterpret_cast<decltype(SetMeshGroupMask)>(FindPattern(CLIENT_DLL, CS_XOR("48 89 5C 24 ? 48 89 74 24 ? 57 48 83 EC ? 48 8D 99 ? ? ? ? 48 8B 71")));
	bSuccess &= (SetMeshGroupMask != nullptr);

	fnSetDynamicAttributeValueUint = reinterpret_cast<decltype(fnSetDynamicAttributeValueUint)>(FindPattern(CLIENT_DLL, CS_XOR("48 89 6C 24 20 57 41 56 41 57 48 81 EC 90 00 00 00 48 8B FA C7 44 24 20 00 00 00 00 4D 8B F8 4C 8D 0D ?? ?? ?? ?? 48 8B E9 4C 8D 05 ?? ?? ?? ?? 33 D2 48 8B 4F 18 E8 ?? ?? ?? ?? 4C 8B F0 48 85 C0 0F 84 9A")));
	bSuccess &= (fnSetDynamicAttributeValueUint != nullptr);

	fnDispatchEffect = reinterpret_cast<decltype(fnDispatchEffect)>(FindPattern(CLIENT_DLL, CS_XOR("48 89 5C 24 08 57 48 81 EC 50 01 ?? ?? 48 8B F9")));
	bSuccess &= (fnDispatchEffect != nullptr);

	GetHashSeed = reinterpret_cast<decltype(GetHashSeed)>(MEM::FindPattern(CLIENT_DLL, CS_XOR("48 89 5C 24 ? 57 48 81 EC ? ? ? ? F3 0F 10 0A")));
	bSuccess &= (GetHashSeed != nullptr);
...
void* MEM::FindHudElement(const char* element) {
	using fn_t = void* (*)(const char* element);
	// x-ref: #STR: "[%d] Could not find Hud Element: %s\n"
	// x-ref to relative: #STR: "CCSGO_HudChat", "#GameUI_VoiceAbusePenaltyNag" -> second function called, only function with "CCSGO_HudChat" as an arg
	using fnFindHud = std::uintptr_t* (__fastcall*)(const char* str);
	static fnFindHud FindHud = reinterpret_cast<fnFindHud>(MEM::FindPattern(CLIENT_DLL, "40 55 48 83 EC ? 48 83 3D"));
	return FindHud(element);
}

void MEM::SendMessageToLocalPlayer(const char* msg) {
	//using fn_t = void(*)(void* chat, int unk, const char* msg);
	//// x-ref to relative: #STR: "CCSGO_HudChat", "#GameUI_VoiceAbusePenaltyNag" -> first call after GameUI_VoiceAbusePenaltyNag
	//static const fn_t fn = reinterpret_cast<fn_t>(MEM::FindPattern(CLIENT_DLL, CS_XOR("48 8B CB E8 ? ? ? ? 48 8D 4C 24 ? E8 ? ? ? ? 48 8B 5C 24 ?")));
