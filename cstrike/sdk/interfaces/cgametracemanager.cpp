...
SurfaceData_t* GameTrace_t::GetSurfaceData()
{
	...
	static fnGetSurfaceData oGetSurfaceData = reinterpret_cast<fnGetSurfaceData>(MEM::GetAbsoluteAddress(MEM::FindPattern(CLIENT_DLL, CS_XOR("E8 ? ? ? ? 80 78 16 00")), 0x1, 0x0));
...
bool game_trace_t::DidHitWorld() const
{
	C_BaseEntity* pWorldEntity = I::GameResourceService->pGameEntitySystem->Get(0);
	if (!pWorldEntity)
		return false;

	return reinterpret_cast<C_BaseEntity*>(HitEntity) == pWorldEntity;
}
