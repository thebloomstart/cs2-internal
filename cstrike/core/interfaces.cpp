...
const CInterfaceRegister* I::GetRegisterList(const wchar_t* wszModuleName)
{
...
#pragma region interface_game_exported
	...
	const auto pParticleRegister = GetRegisterList(PARTICLES_DLL);
	if (pParticleRegister == nullptr)
		return false;

	ParticleManager = Capture<c_particle_manager>(pParticleRegister, PARTICLE_MGR);
...
#pragma endregion
	...
	//GameRules = *reinterpret_cast<CGameRules**>(MEM::GetAbsoluteAddress(MEM::FindPattern(CLIENT_DLL, CS_XOR("48 8B 0D ?? ?? ?? ?? 48 85 C9 74 ?? 48 8B 01 48 8D 54 24 ?? 45 33 C0")), 0x3));
	...
	//const auto pPanoramaSystemRegisterList = GetRegisterList(L"panorama.dll");
	//if (pPanoramaSystemRegisterList == nullptr)
	//	return false;

	//if (!PanoramaUIEngine)
	//	PanoramaUIEngine = Capture<CPanoramaUIEngine>(pPanoramaSystemRegisterList, "PanoramaUIEngine001");
	//bSuccess &= (PanoramaUIEngine != nullptr);
